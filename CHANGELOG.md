# Changelog — gregMod.Economics

Format: [Keep a Changelog](https://keepachangelog.com/de/1.0.0/). Version: siehe [`VERSION`](VERSION).

## [Unreleased]

### Added

- Konfigurierbare Hotkeys (`MarketKey` Default F11, `DiagnoseKey` Default F10), Mod-Vertrag, Tasten-HUD-Eintrag und Oeffner fuers F1-Hub (nur mit gregCore).
- Einheitliches Open-Source-Layout (README, Docs, Badges) nach gregCore-Vorbild.

## [0.1.0] — 2026-09-22

- Initialer standardisierter Stand.
