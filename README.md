...
> A lightweight dynamic marketplace for Data Center production capacity.
...
```bash
dotnet build gregMod.EconomyEngine.csproj -c Release -p:Platform=x64
```

## Project Structure

Source is under `src/`, current game references under `references/`, and maintenance documentation under `docs/`. The current build is standalone and no longer requires the outdated gregCore plugin API.
